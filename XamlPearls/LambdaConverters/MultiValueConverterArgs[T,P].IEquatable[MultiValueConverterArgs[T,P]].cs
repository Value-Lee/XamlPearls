...
namespace XamlPearls.LambdaConverters
{
    partial struct MultiValueConverterArgs<T, P> : IEquatable<MultiValueConverterArgs<T, P>>
    {
        ...
        public override int GetHashCode()
            => (values?.Aggregate(0, (a, item) => a ^ (item != null ? EqualityComparer<T>.Default.GetHashCode(item) : 0)) ?? 0) ^
                (Parameter != null ? EqualityComparer<P>.Default.GetHashCode(Parameter) : 0) ^
